...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Respuesta: \texttt{+OK ACT:<n> HIST:<n> BYTES:<n>\textbackslash r\textbackslash n}
    ...
    \begin{itemize}
        ...
        \item Respuesta: \texttt{+OK\textbackslash r\textbackslash n} o
              \texttt{-ERR <motivo>\textbackslash r\textbackslash n}
    ...
    \begin{itemize}
        ...
        \item Respuesta: \texttt{+OK\textbackslash r\textbackslash n} o
              \texttt{-ERR user not found\textbackslash r\textbackslash n}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Respuesta: \texttt{+OK\textbackslash r\textbackslash n}
    \end{itemize}
\end{enumerate}
...
Se entregó un proxy SOCKS5 monohilo funcional sobre I/O no bloqueante multiplexada,
con autenticación RFC~1929 opcional según usuarios registrados, destinos IPv4/IPv6/FQDN,
reporte de errores del protocolo, graceful shutdown y un protocolo de management
propio con autenticación por secreto compartido.

El diseño alrededor de una máquina de estados por conexión y buffers circulares
permitió manejar lecturas/escrituras parciales y el túnel COPY de forma ordenada.
Las pruebas de estrés muestran que se cumple el piso de 500 conexiones concurrentes
de la consigna; el límite práctico actual está ligado a la capacidad del selector.

Como aprendizaje principal queda la importancia de no introducir round-trips
artificiales en el event loop (write optimista) y de documentar con precisión el
protocolo de administración: terminadores, autenticación y semántica de métricas
frente a conceptos del SO como el backlog de \texttt{listen}.
...
\begin{itemize}
    \item Linux con GCC compatible con C11, GNU Make y pthreads.
\end{itemize}
...
Artefactos: \texttt{bin/server}, \texttt{bin/client}. Objetos en \texttt{obj/}.
...
./bin/server [OPCIONES]
  ...
  -A <secreto>     Secreto de management (defecto: changeme).
  -u <usr>:<pass>  Usuario inicial (hasta 10).
  -v               Versión.
\end{lstlisting}
...
\subsection{Escenario 1: monitoreo local}

\begin{lstlisting}
$ ./bin/server -A mi_secreto -u admin:admin123
...
$ ./bin/client -A mi_secreto add-user desarrollador pass123
usuario agregado
...
$ ./bin/client -A mi_secreto get-metrics
conexiones activas: 0
conexiones historicas: 1
bytes transferidos: 2048
\end{lstlisting}

\subsection{Escenario 2: auditoría de accesos}
